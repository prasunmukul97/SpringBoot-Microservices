...
public class WebSecurity extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception{
		...
		.antMatchers(env.getProperty("api.users.actuator.url")).permitAll()
		...
		.and()
		.addFilter(new AuthorizationFilter(authenticationManager(),env)); //setting authentication manager during authorization filter registration.
		
		http.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS); //It will make our API stateless.
	}
